...
import styles from "../../app/browser/page.module.css";
...
export function BrowserContent() {
  ...
  const paginatedMemories = memories.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
        ...
        <GlassCard className={styles.tableCard}>
          {searchMode && memories.length > 0 && (
            <div className={styles.searchBanner}>
              <Banner variant="info" darkMode={darkMode}>
                <strong>Search Results:</strong> Showing {memories.length} memories ranked by
                semantic relevance to &quot;{searchQuery}
                &quot;
              </Banner>
            </div>
          )}
            ...
            <>
              <div className={styles.tableWrap}>
                <table className={styles.table}>
                  <thead>
                    <tr>
                      {searchMode && <th style={{ width: "10%" }}>Relevance</th>}
                      <th style={{ width: searchMode ? "40%" : "50%" }}>Text</th>
                      <th>Tags</th>
                      <th>Layer</th>
                      <th>Type</th>
                  ...
                  <tbody>
                    {paginatedMemories.map((memory) => (
                      ...
                      >
                        ...
                        <td>
                          <div className={styles.cellTags}>
                            {memory.tags.slice(0, 3).map((tag) => (
                              <Chip key={tag} label={tag} variant="blue" darkMode={darkMode} />
                            ))}
                        ...
                        <td>
                          {memory.memoryType ? (
                            <Chip label={memory.memoryType} variant="yellow" darkMode={darkMode} />
                          ) : (
                            <span style={{ opacity: 0.3, fontSize: "0.8rem" }}>—</span>
                          )}
                        </td>
                              ...
                              style={{
                                ...
                                color:
                                  memory.confidence >= 0.7
                                    ? "var(--lg-color-green-dark1, #00a35c)"
                                    : memory.confidence >= 0.4
                                      ? "var(--lg-color-yellow-dark2, #944f01)"
                                      : "var(--lg-color-red-dark1, #cf4747)",
                              }}
                            >
