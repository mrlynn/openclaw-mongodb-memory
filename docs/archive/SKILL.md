...
_(To be integrated into OpenClaw startup)_
...
// Store a memory
const id = await memory.remember("The user prefers Material UI for design systems", {
...
const results = await memory.recall("What UI framework does the user like?", {
  limit: 5,
