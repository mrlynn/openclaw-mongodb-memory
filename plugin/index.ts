...
async function remember(daemonUrl: string, agentId: string, text: string, tags: string[] = [], ttl?: number) {
...
export default function createPlugin(api: any) {
  const config = {
    daemonUrl: api.config.plugins?.entries?.['openclaw-memory']?.config?.daemonUrl || 'http://localhost:7751',
    ...
    autoStartDaemon: api.config.plugins?.entries?.['openclaw-memory']?.config?.autoStartDaemon !== false,
    middlewareEnabled: api.config.plugins?.entries?.['openclaw-memory']?.config?.middlewareEnabled === true,
  };
  ...
  api.registerTool({
    name: 'memory_search',
    description: 'Semantically search long-term memory stored in MongoDB. Use this to recall prior decisions, preferences, context, or facts.',
    parameters: {
      type: 'object',
      properties: {
        query: {
    ...
    async execute(_id: string, params: { query: string; maxResults?: number }) {
      try {
        ...
        const resultText = filtered.map((r: any) => 
          `[Score: ${r.score.toFixed(3)}] ${r.text}\nTags: ${r.tags.join(', ')}\n`
        ).join('\n');
  ...
  api.registerTool({
    name: 'memory_get',
    description: 'Read a specific memory file from the workspace. Use memory_search for semantic recall; use this for targeted file reads.',
    parameters: {
      type: 'object',
      properties: {
        path: {
    ...
    async execute(_id: string, params: { path: string; from?: number; lines?: number }) {
      try {
        const workspace = api.config.agents?.defaults?.workspace || process.env.HOME + '/.openclaw/workspace';
  ...
  return {
    api: {
      recall: (query: string, limit?: number) => recall(config.daemonUrl, config.agentId, query, limit || config.maxResults),
      remember: (text: string, tags?: string[], ttl?: number) => remember(config.daemonUrl, config.agentId, text, tags, ttl),
